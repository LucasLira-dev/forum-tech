...
import { UserInfoCardSkeleton } from '@/components/Skeletons/userInfoSkeleton';
import { MappedUser } from './types';

interface UserManagementProps {
  isLoading: boolean;
  usersRecentes: MappedUser[];
  admins: MappedUser[];
  bannedUsers: MappedUser[];
}


export const UserManagement = ({ isLoading, usersRecentes, admins, bannedUsers }: UserManagementProps) => {
          ...
          <Tabs.Content value="recent" className="mt-4">
            <div className=" flex flex-col gap-2">
              {isLoading ? (
                ...
                usersRecentes.map((user: MappedUser) => (
                  <UserInfoCard
                    key={user.id}
                    id={user.id}
            ...
            <div className="flex flex-col gap-2">
              {isLoading ? (
                ...
                bannedUsers.length > 0 ? (
                  bannedUsers.map((banned: MappedUser) => (
                    <UserInfoCard
                      key={banned.id}
                      id={banned.id}
                      userAvatar={banned.userAvatar ?? ""}
                      userName={banned.userName ?? banned.name}
                      isAdm={false}
