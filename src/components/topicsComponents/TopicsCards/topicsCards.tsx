...
export const TopicsCards = ({id, title, description, user, time, answers }: TopicsCardsProps   ) => {
    ...
    return (
      <div className="flex flex-col gap-4 p-4 rounded-lg mb-4 hover:bg-[var(--muted)] hover:text-[var(--muted-foreground)] transition-colors border border-[var(--border)] cursor-pointer">
        <Link href={`/topics/${id}`}>
          <h2 className="text-[var(--foreground)] font-semibold text-lg">
            {title}
          </h2>
